...
/// List all tickets (paginated with filters)
#[utoipa::path(
    get,
...
pub async fn list_tickets(
    ...
    let (items, total) = service.list_tickets(&params).await?;
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Ticket details", body = ApiResponse<AdminTicketDetailDto>),
        (status = 401, description = "Unauthorized"),
        (status = 403, description = "Forbidden - Super admin access required"),
...
pub async fn get_ticket(
    ...
    State(service): State<Arc<AdminService>>,
    Path(id): Path<Uuid>,
) -> Result<Json<ApiResponse<AdminTicketDetailDto>>> {
    let item = service.get_ticket(id).await?;
    Ok(Json(ApiResponse::success(Some(item), None, None)))
}
